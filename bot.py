...
from config import BOT_TOKEN, DATABASE_URL, OPENROUTER_API_KEY, ADMIN_IDS
from database import create_tables, get_connection, release_connection, db_connection
...
def check_session_limit(telegram_id):
    ...
    with db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT sessions_today, last_session_date, is_subscribed, subscription_end
    ...
    sessions_today, last_session_date, is_subscribed, subscription_end = row

    if is_subscribed and subscription_end and subscription_end >= datetime.now().date():
        return True
    ...
    return sessions_today < FREE_SESSIONS_LIMIT

def increment_session_count(telegram_id):
    if telegram_id in ADMIN_IDS:
        return
    ...
    with db_connection() as conn: 
        cursor = conn.cursor()
        cursor.execute("SELECT last_session_date FROM users WHERE telegram_id = %s", (telegram_id,))
        row = cursor.fetchone()
...
def handle_buy_subscription(call):
    ...
    prices = [telebot.types.LabeledPrice(label="Подписка на 30 дней", amount=SUBSCRIPTION_PRICE)]

    bot.send_invoice(
        telegram_id,
        title="Подписка YourReflection",
        description="Безлимитный доступ к дневнику на 30 дней",
...
def handle_succesfuly_payment(message):
    telegram_id = message.from_user.id
    ...
    with db_connection() as conn:
        ...
        cursor.execute("""
            UPDATE users
            SET is_subscribed = TRUE, subscription_end = %s 
            WHERE telegram_id = %s
        """, (subscription_end, telegram_id))
        conn.commit()
        cursor.close()

    print(f"Оплата: {telegram_id}, payment_id: {payment_id}")

    bot.send_message(telegram_id, "Подписка активирована! 🎉\nБезлимитный доступ на 30 дней.")
    show_main_menu(telegram_id)
...
## Лёгкие сообщения без запроса на анализ

Если пользователь просто делится настроением, рассказывает о дне или пишет что-то нейтральное без явной проблемы — НЕ запускай анализ. Откликнись тепло в одном-двух предложениях. Не развивай этот диалог — не задавай уточняющих вопросов, не предлагай тему для разбора. Просто дай понять, что ты услышал. Например, на «сегодня гуляла с собакой, было хорошо» достаточно: «Рада за вас. Если захотите что-то обсудить — я здесь.» Переходи к анализу только если пользователь сам поднимает проблему.
...
def start_diary(telegram_id):
    ...
    if row and row[0]:
        ...
        return 
    if not check_session_limit(telegram_id):
        show_paywall(telegram_id)
        return
